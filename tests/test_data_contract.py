...
def test_split_json_indices_are_positional(tmp_path):
    root = _make_root(tmp_path)
    split = tmp_path / "split.json"
    split.write_text(json.dumps({"test": {"indices": [0]}, "train": {"indices": []}}))
    cfg = _cfg(root); cfg.test_indices = str(split)
    assert len(MeshUVDataset(cfg, split="test")) == 1
