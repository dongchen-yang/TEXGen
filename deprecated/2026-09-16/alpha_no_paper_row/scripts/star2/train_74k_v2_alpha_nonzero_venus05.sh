#!/bin/bash
# TEXGen alpha _v2 NONZERO on cs-venus-05 — the alpha run trained on the EMISSIVE-ONLY
# training set (zero-emission shapes removed from train; val/test unchanged).
# 1 node x 4 RTX PRO 6000 Blackwell Max-Q, per-GPU batch 32 -> GLOBAL BATCH 128.
#
# A verbatim copy of train_74k_v2_alpha_venus05.sh with THREE substitutions: NAME,
# CONFIG, and the split filename in the data pre-flight (plus its own ATTEMPTS
# counter, so a crash here does not consume the baseline run's retry budget).
...
# guard, GPUS=0,1,2,3, MEM=400G — is unchanged, because every one of those lines
# encodes a failure this run would otherwise repeat.
#
...
NAME=texgen_alpha_74k_v2_nonzero
CONFIG=${CONFIG:-configs/lightgen_pointuv_256_batch32_emissive_74k_v2_alpha_venus05_nonzero.yaml}
EXTRA=${EXTRA:-}
...
ATTEMPTS=${PROJECT}/.texgen_v2_alpha_nonzero_attempts
MAX_ATTEMPTS=3
...
for f in ${PROJECT}/data/df_SomgProc_final.parquet \\
         ${PROJECT}/data/data_splits_emissive_74k_stratified_newbake_vae_nonzero.json \\
