...
def main(args, extras) -> None:
    ...
    from spuv.utils import launch_ext
    from spuv.utils.wandb_utils import build_wandb_logger
    launch_ext.patch_torch_load()
    launch_ext.register_safe_globals()
    ...
    launch_ext.resolve_auto_resume(cfg, train=args.train)
    wandb_run_id, resume_step, resume_epoch = launch_ext.read_resume_info(
        cfg.resume if args.train else None, want_wandb=args.wandb)
    ...
    if args.train:
        callbacks += [
            ModelCheckpoint(
                dirpath=launch_ext.checkpoint_dirpath(cfg), **launch_ext.checkpoint_kwargs(cfg)
            ),
    ...
    if args.train:
        ...
        if args.wandb:
            wandb_logger = build_wandb_logger(cfg, wandb_run_id, system)
    ...
    if args.train:
        ...
        launch_ext.fit_with_graceful_exit(trainer, system, dm, cfg.resume, args.wandb, resume_epoch, resume_step)
        if args.gradio:
